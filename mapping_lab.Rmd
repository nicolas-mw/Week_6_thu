...
unintentional_injuries <- unintentional_injuries %>% clean_names()
...
```{r}
filtered_data <- unintentional_injuries %>% 
  filter(injury_type == "Falls", sex == "Female", age_group == "75plus years", injury_location == "Home", financial_year == "2022/23", hbr != "S92000003")

falls_admissions_75_summary <- filtered_data %>% 
  group_by(hbr) %>% 
  summarise(sum_admissions = sum(number_of_admissions)) 
...
joined_data <- full_join(NHS_healthboards, falls_admissions_75_summary, by = c("HBCode" = "hbr"))
...
```{r}
joined_data %>% 
  ggplot(aes(fill = sum_admissions)) +
  geom_sf() +
  scale_fill_distiller(palette = "Spectral") +
  labs(title = "Number of falls at home in Scotland", subtitle = "females age 75+ in 2022/23") + 
   theme_void() 
...
hb2019_pop_est <- read_csv("data/hb2019_pop_est.csv")

a <- hb2019_pop_est %>% 
  filter(Year == 2022, Sex == "Female", HB != "S92000003") %>% 
  select(!Age0:Age69) %>% 
  mutate(total_pop = rowSums(across(Age70:Age90plus))) 

join_data2 <- full_join(a, falls_admissions_75_summary, by = c("HB" = "hbr"))

join_data2 <- join_data2 %>% 
  mutate(falls_per_10k = (sum_admissions / total_pop) * 10000)

join_data3 <- full_join(NHS_healthboards, join_data2, by = c("HBCode" = "HB"))
...
join_data3 %>% 
  ...
  scale_fill_distiller(palette = "Spectral") + 
  theme_void() + 
  labs(title = "Falls per 10K across Scotland", subtitle = "Females, 75+, 2022")
...
join_data3 %>% 
  ...
  scale_fill_distiller(palette = "Spectral") + 
  theme_void() + 
  labs(title = "Falls per 10K across Scotland", subtitle = "Females, 75+, 2022") + annotation_scale(    
    location = "tl"
  ) +
